...
simulated function ApplyEffectToWorld(const out EffectAppliedData ApplyEffectParameters, XComGameState NewGameState)
{
	...
	if (ApplyChancePerTile != 0)
	{
		for (i = DamageEvent.DamageTiles.Length - 1; i >= 0; i--)
		{
			if (`SYNC_RAND(100) >= ApplyChancePerTile)
			{
				DamageEvent.DamageTiles.Remove(i, 1);
			}
		}
	}
}
